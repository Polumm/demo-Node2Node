...
PRESENCE_SERVICE_URL = os.getenv(
    "PRESENCE_SERVICE_URL", "http://presence-service:8004"
)
...
async def node2node_loop():
    ...
    while True:
        try:
            ...
            print(
                f"[{NODE_ID}] Connected to node2node-service at {connect_url}."
            )
            ...
            while True:
                ...
                print(
                    f"[{NODE_ID}] Received message from {source_node}: {payload}"
                )

                # Deliver to local WebSocket if connected.
                if recipient_id in connected_users:
                    await connected_users[recipient_id].send_text(
                        json.dumps(
                            {
                    ...
                    print(
                        f"[{NODE_ID}] User {recipient_id} not connected locally."
                    )
        except Exception as e:
            print(
                f"[{NODE_ID}] Lost connection to node2node-service: {e}, retry in 5s..."
            )
...
async def websocket_user(websocket: WebSocket, user_id: str):
    ...
    try:
        ...
        if resp.status_code == 200:
            print(
                f"[{NODE_ID}] Registered presence for user {user_id} on {NODE_ID}"
            )
        else:
            print(
                f"[{NODE_ID}] Failed to register presence for user {user_id}: {resp.text}"
            )
    except Exception as e:
        print(
            f"[{NODE_ID}] Exception during presence registration for {user_id}: {e}"
        )
    ...
    try:
        while True:
            ...
            resp = requests.get(
                f"{PRESENCE_SERVICE_URL}/presence/{recipient_id}"
            )
            if resp.status_code != 200:
                print(
                    f"[{NODE_ID}] Presence lookup failed for {recipient_id}: {resp.text}"
                )
                continue  # or you could send an error message back to the sender

            presence = resp.json()
            recipient_node = presence.get("node_id")
            if not recipient_node:
                print(
                    f"[{NODE_ID}] Received invalid presence data for {recipient_id}: {presence}"
                )
                continue

            print(
                f"[{NODE_ID}] Presence says {recipient_id} is on {recipient_node}"
            )
            ...
            if recipient_node == NODE_ID:
                ...
                if recipient_id in connected_users:
                    ...
                    print(
                        f"[{NODE_ID}] Recipient {recipient_id} not connected locally."
                    )
            else:
                ...
                if node2node_ws is not None:
                    ...
                    print(
                        f"[{NODE_ID}] Forwarding via node2node-service: {msg_to_send}"
                    )
                    await node2node_ws.send(msg_to_send)
                else:
                    print(
                        f"[{NODE_ID}] ERROR: node2node_ws is None; cannot forward."
                    )
